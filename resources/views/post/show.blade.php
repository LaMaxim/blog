                            ...
                            <div v-for="comment in comments">
                                <div class="comment">
                                    ...
                                    <div class="comment-value">
                                        <span v-bind:id="'text-comment-' + comment.id" class="text-comment">{{comment.text}}</span>
                                        <div v-bind:id="'edit-form-' + comment.id" class="edit-form">
                                            <div id="comment-form">
                                            <textarea v-bind:id="'edit-textarea-comment-' + comment.id"
                                                      v-bind:value="comment.text">
                                            </textarea>
                                            </div>
                                            <div v-bind:id="'option-edit-panel-' + comment.id">
                                                <button class="btn btn-primary btn-xs" @click="clickSendEditComment(comment.id)">Send</button>
                                                <button class="btn btn-default btn-xs" @click="clickCloseEditComment(comment.id)">Close</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div v-if="currentUser && currentUser.roles[0].name === EDITOR_ROLE"
                                         v-bind:id="'option-btn-container' + comment.id">
